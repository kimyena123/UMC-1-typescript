...
const renderTask = (): void => {
  ...
  doneTasks.forEach((todo: Todo): void => {
    const doneItem = createTodoElement(todo, true);
...
const completeTodo = (todo: Todo): void => {
  todos = todos.filter((t: Todo): boolean => t.id !== todo.id);
  doneTasks.push(todo);
  renderTask();
};
...
const deleteTodo = (todo: Todo): void => {
  doneTasks = doneTasks.filter((t: Todo): boolean => t.id !== todo.id);
  renderTask();
};
